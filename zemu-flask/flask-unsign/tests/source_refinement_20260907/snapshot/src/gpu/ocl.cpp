...
#define RESOLVE(name) \
  ...
  RESOLVE(clFlush) RESOLVE(clFinish) RESOLVE(clReleaseEvent)
...
static OclCtx& ocl() {
  ...
  const uint32_t CACHE_MAGIC = 0x5A4B4E14; // "ZKN" v20:回退 v19 免展开压缩实验,保留 v18 掩码 key 打包特化;递增版本以使实验缓存失效
 ...
 *  混合字典(hyb + rawMap)时 rawSpaceTotal 为原字典词数(含打包跳过的词);
 *  其余情形忽略该参数,raw 空间即 total。 */
static int runChunks(OclCtx& c, cl_kernel k, cl_mem foundBuf, uint64_t total, int baseArgIdx,
  ...
  auto claim = [&](Chunk& out) {
    ...
    // (rawSpaceTotal,固定值)。必须在 raw 空间切块,再映射到 packed 区间,
    // 否则跳过词后面的候选会漏查。
    //
    // 认领方唯一(GPU 调度循环是单线程),用 load-检查-store 即可。
    // 关键不变量:head 只能推进到「实际将要入队验证」的边界——CPU 侧依据 head
    // 放弃区间(end <= head 整块放弃;start < head 丢弃前缀),head 若超过已承诺
    // 验证的位置(例如被整块 fetch_add 推进而 rawHi 被 tail 钳小),被放弃区间
    // 内的词就两侧都不会验(实测漏查)。
    const uint64_t rawTotal = (hyb && rawMap) ? rawSpaceTotal : total;
    uint64_t lo;
    if (hyb) {
      if (hyb->stop.load(std::memory_order_relaxed)) return false;
      lo = hyb->head.load(std::memory_order_relaxed);
      if (lo >= rawTotal) return false;
      uint64_t hi = lo + CHUNK_CAND < rawTotal ? lo + CHUNK_CAND : rawTotal;
      const uint64_t t = hyb->tail.load(std::memory_order_relaxed);
      if (lo >= t) return false; // 与 CPU 认领前沿会合,剩余空间归 CPU
      if (hi > t) hi = t;        // 钳到 CPU 前沿:边界重叠只是重复验(幂等),钳小是为省时
      hyb->head.store(hi, std::memory_order_relaxed);
      out.rawLo = lo;
      out.rawHi = hi;
    } else {
      lo = cursor;
  ...
  auto prefetch = [&](const Chunk& ch, cl_event& ev) {
    ...
    // 跨队列事件必须 flush 生产队列:q 上的 kernel 等待 ev,但不保证驱动会把
    // q2 里的 copy 提交给设备(规范 §5.13;当前 NVIDIA 驱动即时提交只是实现巧合,
    // 严格合规驱动下消费方可能永远等不到事件)。flush 不等待完成,只保证已下发。
    if (rc == CL_SUCCESS) rc = p_clFlush(c.q2);
    return rc;
  };
...
int gpuCrackDict(const GpuCrackParams& p, const uint8_t* words, size_t stride, uint64_t count,
                 ...
                 OclHostBuf* pinned, uint64_t rawTotal) {
  ...
  int rc = runChunks(c, k, bFound.m, count, 11, 12, foundIdx, err, hybrid, rawMap,
                     attemptsOut, usePinned ? &pipe : nullptr, rawTotal);
  if (getenv("ZK_PROF")) {
