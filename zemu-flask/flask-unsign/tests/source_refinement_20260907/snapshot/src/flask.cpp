...
// 启用条件:常驻模式(flaskSetResident)或显式设置 ZK_DICTCACHE_MB;CLI 单发进程
// 默认不写缓存(回填 idxMap/packed 的拷贝没有复用机会,实测拖慢约 4~9%)。
// 内存预算:ZK_DICTCACHE_MB(默认 1024 MiB,0 = 禁用),按 vector 实际容量计费。
// 单进程单线程命令循环使用,无并发访问。
struct DictCacheEntry {
  ...
  uint64_t bytes() const { // 按实际持有容量计费:size 会低估(loadWordSet 紧凑化后 capacity 可能残留)
    return (uint64_t)words.bytes.capacity() + words.off.capacity() * 4 +
           idxMap.capacity() * 8 + skippedIdx.capacity() * 8 + packed.capacity();
  }
};
static DictCacheEntry g_dictCache;
static bool g_dictCacheValid = false;
static bool g_resident = false; // serve / interactive 由 main 置位

void flaskSetResident(bool r) { g_resident = r; }
...
/** 缓存是否启用:常驻模式默认启用;CLI 单发默认关闭,显式设 ZK_DICTCACHE_MB 可强制 */
static bool dictCacheOn() {
  if (g_resident) return dictCacheBudget() != 0;
  return std::getenv("ZK_DICTCACHE_MB") != nullptr && dictCacheBudget() != 0;
}
...
static DictCacheEntry* dictCacheLookup(const std::string& path) {
  if (!dictCacheOn() || !g_dictCacheValid || g_dictCache.path != path) return nullptr;
  uint64_t sz;
  int64_t mt;
...
static DictCacheEntry* dictCacheStore(const std::string& path, WordSet& words,
  ...
  if (!dictCacheOn()) return nullptr;
  // 按实际容量计费(loadWordSet 已收缩显著浪费,残余冗余有限)
  if ((uint64_t)words.bytes.capacity() + words.off.capacity() * 4 > budget) return nullptr;
    ...
    if (!idxMap.empty()) {
      if (engine == "auto" && !cudaAuto) {
        ...
        if (rc == -1) {
          ...
          // 补验的耗时与次数计入最终统计,避免长词较多时低报
          attempts += rechecked;
          secs = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
          rate = secs > 0 ? attempts / secs : 0;
          std::cerr << "[*] 引擎 GPU+CPU,跑完未命中,耗时 " << secs << " s,约 "
                    << (uint64_t)rate << "/s" << std::endl;
          return 1;
        }
      ...
      if (rc == 1) {
        ...
        std::cerr << "[*] 引擎 " << (cudaBe ? "GPU-CUDA" : "GPU") << ",跑完未命中,耗时 " << secs << " s,约 "
                  << (uint64_t)(secs > 0 ? (idxMap.size() + skippedIdx.size()) / secs : 0) << "/s" << std::endl;
        return 1;
      }
