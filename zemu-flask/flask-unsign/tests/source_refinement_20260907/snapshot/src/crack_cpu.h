...
/**
 * 连续字典表示:所有词字节紧密排列(无分隔符)+ 偏移索引。
 * 词 i = bytes[off[i], off[i+1]);off 比词数多一个元素。
 * 相比 vector<string>:加载无逐行 string 分配(实测 1600 万词快 ~75%),
 * 峰值内存约减半;views 在加载完成后稳定(bytes 不再重分配)。
 */
struct WordSet {
  std::vector<char> bytes;
  std::vector<uint32_t> off; // 词数+1 个偏移;词长 = off[i+1]-off[i](u32:文件 ≤ 4GB)
  uint32_t maxLen = 0;

  size_t size() const { return off.empty() ? 0 : off.size() - 1; }
  bool empty() const { return size() == 0; }
  std::string_view at(size_t i) const {
    return {bytes.data() + off[i], (size_t)(off[i + 1] - off[i])};
  }
  const uint8_t* data(size_t i) const { return (const uint8_t*)bytes.data() + off[i]; }
  size_t len(size_t i) const { return (size_t)(off[i + 1] - off[i]); }
};

/** 加载字典文件为连续表示(去行尾 \r、跳过空行);失败置 error。超过 4GB 拒绝(u32 偏移上限)。 */
