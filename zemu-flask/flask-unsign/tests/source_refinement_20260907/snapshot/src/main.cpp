...
static void usage(std::ostream& os, bool color) {
  ...
  os << B << "用法:" << R << "\n"
     ...
     << " [" << YE << "--salt <s>" << R << "] [" << YE << "--max-age <秒>" << R
     << "] [" << YE << "--legacy" << R << "]  验证 cookie 签名\n"
     << "    默认 valid 仅表示签名匹配,不检查时间或保证目标应用接受。\n"
     << "    --max-age 为非负整数:另外检查时间戳、未来时间及最大年龄;--legacy 使用旧版 epoch。\n"
     << "    输出 valid/invalid;启用时间检查另有 expired/future/invalid-timestamp。\n"
  ...
  if (cmd == "flask") {
    ...
    for (int i = 3; i < argc; i++) {
      ...
      if (a == "--cookie") cookie = next();
        ...
        int64_t parsed = 0;
        auto result = std::from_chars(v.data(), v.data() + v.size(), parsed);
        if (v.empty() || result.ec != std::errc{} || result.ptr != v.data() + v.size() || parsed < 0) {
          std::cerr << "[!] --max-age 须为非负整数秒数(0..9223372036854775807)" << std::endl;
          return 2;
        }
        maxAge = parsed;
      }
    ...
    if (maxAge >= 0 && action != "verify") {
      std::cerr << "[!] --max-age 仅适用于 flask verify" << std::endl;
      return 2;
    }
    if (action == "decode" && !cookie.empty()) return flaskDecode(cookie);
    if (action == "verify" && !cookie.empty() && !secret.empty())
      return flaskVerify(cookie, secret, salt, maxAge, legacy);
...
static int cmdServe() {
  ...
  flaskSetResident(true); // 常驻进程:启用字典缓存(跨请求复用加载与打包产物)
  std::string line;
  while (std::getline(std::cin, line)) {
    ...
    if (ok) {
      parser.ws();
      // Compare the full length, not just *p: raw NUL must not hide a suffix.
      ok = parser.p == line.c_str() + line.size();
    }
    if (ok) {
...
static int cmdInteractive() {
  flaskSetResident(true); // 交互模式同样常驻:启用字典缓存
