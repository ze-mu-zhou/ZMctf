...
`--mode match` 且长度 ≤ 55 时默认自动启用 GPU（`--gpu` / `--no-gpu` 控制，失败自动回退 CPU 多线程）。实现零 SDK 依赖：运行时动态加载显卡驱动自带的 `OpenCL.dll`，内核源码在首次搜索时按（长度、目标类型）现场编译。检测到多颗 GPU 时全部启用（含核显）：每颗设备一条主机线程、独立 context，从共享根游标动态领取按 ~200ms 自适应伸缩的任务块，慢卡编译期间由屏障对齐起跑，块大小初值按算力单元数 × 时钟加权。

内核结构移植自 hashcat（MIT 许可证）的 `m00000_a3-optimized` 单目标内核：

- 每个 GPU 线程从全局索引自行混合进制解码出 root 候选（尾部位置）——主机零解码、零逐 root 上传，单次启动即可覆盖全空间；
- 内层循环只翻消息字 0（候选前 4 字节），来自主机预算的 OR 表，每线程 16 候选（`uint16` 向量 ILP；实测 sm_89 上 158 寄存器无 spill，ILP 收益大于占用率损失）；
...
实测（RTX 4060 Laptop + Radeon 核显双卡，长度 10 数字全空间 1e10 候选）：多设备改造后约 27.7 GH/s，超过同机 hashcat 7.1.2 MD5 双后端实测（CUDA 单卡 24.4 GH/s、OpenCL 双卡 26.1 GH/s）；单卡约 25.4 GH/s 与 hashcat 单卡基本持平；CPU 32 线程 AVX-512 约 0.85 GH/s。注意 `--max-candidates` 下 GPU 路径按跨步子集覆盖（数量语义不变，但不保证是字典序前缀）。

调试/调优环境变量：`ZM_DUMP_KERNEL=路径` 导出现场生成的内核源码与 ptxas 寄存器报告（`.log`）；`ZM_VEC=4|8|16`（默认 16）、`ZM_LOCAL=128|256|512`（默认 256）覆盖发射参数用于调优实验；`ZM_GPUS=0,1,...` 按算力排序下标挑选 GPU 子集（默认全部）。

发射参数可以不手工试：`--mode autotune` 会在最快的 GPU 上用固定合成空间（长度 10 数字、完整目标）实测全部 VEC×LOCAL 组合（每个组合 1 轮预热 + 5 轮计时取中位数，约 1 分钟），把最优值按"设备名 + 驱动版本"缓存到 `%LOCALAPPDATA%\zm-hash\autotune.cache`（Linux 为 `$XDG_CACHE_HOME/zm-hash/` 或 `~/.cache/zm-hash/`）。之后的 GPU 搜索自动使用缓存值并打印来源；`ZM_VEC`/`ZM_LOCAL` 优先级最高，仍可用于临时实验。

## 示例

`--mode match` 必须指定 `--target-digest`（完整摘要）或 `--digest-pattern`（`?` 通配模式）之一，否则直接报错退出。
